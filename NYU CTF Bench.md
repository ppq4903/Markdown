...
| 贡献类别         | 具体内容                                                     | 关键价值                                                     |
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 开源基准数据集   | 1. 源自 NYU 年度 CSAW CTF 竞赛（2017-2023 年），共 200 个验证通过的挑战，覆盖 6 大安全领域2. 每个挑战包含元数据（名称、描述、所需文件、端口信息）、Docker 部署配置、flag 真值（模型不可见）3. 难度按 “积分” 划分（1-500 分），资格赛难度较低，决赛难度显著更高 | 解决现有数据集规模小、场景单一问题，提供标准化的 CTF 评估场景 |
| 自动化评估框架   | 1. 支持 5 种 LLMs（3 种黑盒模型：GPT-3.5/4、Claude 3；2 种开源模型：Mixtral、LLaMA 3）2. 集成 8 类网络安全工具（如 Ghidra 反编译、netcat 网络连接、SageMath 密码计算等）3. 实现全流程自动化：挑战加载（Docker 容器 / 本地文件）、工具调用、结果验证、日志记录 | 无需人工干预，可重复验证 LLMs 的自主解决能力                 |
| 专用工具集与流程 | 1. 5 个核心模块：后端（对接 OpenAI/Anthropic API 及开源模型服务）、数据加载（Docker 部署 + 临时文件管理）、外部工具（命令执行、文件创建、反编译等）、日志系统（Markdown/JSON 双格式记录）、提示工程（标准化任务描述）2. 针对不同 CTF 类别适配工具（如 pwn/rev 类启用反编译，web 类禁用避免干扰） | 提升 LLMs 工具调用准确性，降低任务无关操作干扰               |
